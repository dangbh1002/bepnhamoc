...
export default {
    ...
    firestore () {
        return {
            ...
            list: db.collection('recipe').where('menuId', '==', this.selectDetail).limit(4)
        }
    },
    computed: {
        selectDetail () {
    ...
    methods: {
        ...
        create () {
            ...
            // upload image
            const imgName = `recipe/${this.selectDetail}/${new Date().getTime()}-${this.imageDataCreate.name}`
        ...
        onUpload (key) {
            ...
            const imgName = `recipe/${this.selectDetail}/${new Date().getTime()}-${this.imageData[key].name}`
        ...
        onDelete (key) {
            ...
            this.$firestore.recipe.doc(key).delete().then(() => {
